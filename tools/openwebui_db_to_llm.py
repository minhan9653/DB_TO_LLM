...
class Tools:
    ...
    def query(self, question: str) -> str:
        """
        다음 상황에서 반드시 이 도구를 사용하라:
        - 데이터베이스에서 데이터를 조회하거나 분석하는 질문 (예: 조회, 집계, 통계, 목록, 현황, 이력)
        - 특정 문서, 매뉴얼, 보고서, 로그 등의 내용을 검색하는 질문
        - DB 데이터와 문서 내용을 함께 참조해야 하는 복합 질문
        - 시스템에 저장된 정보나 기록을 물어보는 모든 질문
        이 도구는 질문 유형을 자동으로 판별하여 DB 조회(SQL), 문서 검색(RAG), 또는 일반 답변으로 처리한다.

        :param question: 사용자의 자연어 질문
        :return: 최종 답변 (생성된 SQL, DB 조회 결과 표, RAG 참조 문서 포함 가능)
        """
