...
def _load_sentence_transformer(model_name: str) -> Any:
    ...
    try:
        ...
        logger.info("SentenceTransformer 모델 로드 완료 (로컬 캐시): %s", model_name)
    except Exception:
        logger.warning("로컬 캐시 로드 실패, 네트워크에서 다운로드 시도: %s", model_name)
        model = SentenceTransformer(model_name)
        logger.info("SentenceTransformer 모델 로드 완료 (다운로드): %s", model_name)
