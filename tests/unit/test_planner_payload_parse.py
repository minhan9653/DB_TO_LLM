...
from src.db_to_llm.stream.planner.plan_validator import validate_plan_payload
...
def test_parse_db_then_rag_plan_fixture(fixture_root: Path) -> None:
    ...
    """
    payload = json.loads(
        (fixture_root / "planner" / "sample_plan_db_then_rag.json").read_text(encoding="utf-8")
    )
...
def test_parse_db_only_plan_fixture(fixture_root: Path) -> None:
    ...
    """
    payload = json.loads(
        (fixture_root / "planner" / "sample_plan_db_only.json").read_text(encoding="utf-8")
    )
    validate_plan_payload(payload)
    plan = PlannerPlan.from_dict(payload)
