# 이 파일은 STREAM 단계 전용 공통 로거 설정을 담당합니다.

# 로그 형식은 시간, 레벨, 파일명, 함수명, 라인번호를 포함합니다.

# 콘솔/파일 로그를 동일한 형식으로 남겨 디버깅을 단순화합니다.

# 다른 모듈은 get_logger()를 통해 동일 설정의 child logger를 사용합니다.
...
def setup_logger(log_level: str = "INFO", log_file_path: Path | None = None) -> logging.Logger:
    ...
    Returns:
    logging.Logger: STREAM 로깅 설정 계산 결과를 `logging.Logger` 타입으로 반환합니다.
...
def get_logger(name: str | None = None) -> logging.Logger:
    """
    역할:
    STREAM 로깅 설정에서 재사용 리소스(로거/컬렉션/모델)를 조회 또는 생성합니다.
