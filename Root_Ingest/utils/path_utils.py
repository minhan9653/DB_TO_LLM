# 이 파일은 경로 처리 규칙을 한곳에 모아둔 유틸입니다.

# 프로젝트 전체에서 pathlib.Path만 사용하도록 강제합니다.

# 상대 경로를 프로젝트 루트 기준 절대 경로로 바꾸는 역할을 합니다.

# 디렉터리 자동 생성 함수도 함께 제공합니다.
...
def get_project_root() -> Path:
    """
    역할:
    INGEST 경로 유틸리티에서 재사용 리소스(로거/컬렉션/모델)를 조회 또는 생성합니다.
    
    Returns:
    Path: INGEST 경로 유틸리티 계산 결과를 `Path` 타입으로 반환합니다.
...
def resolve_path(path_value: str | Path, base_path: Path | None = None) -> Path:
    """
    역할:
    INGEST 경로 유틸리티에서 설정값을 검증 가능한 최종 값으로 확정합니다.
...
def ensure_directory(directory_path: Path) -> Path:
    """
    역할:
    INGEST 경로 유틸리티 문맥에서 `ensure_directory` 기능을 수행합니다.
